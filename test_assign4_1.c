...
testInsertAndFind (void)
{
  ...
  ASSERT_EQUALS_INT(testint,4, "number of nodes in btree"); // Fixed Summer 2021 - Old: ASSERT_EQUALS_INT(4,testint, "number of nodes in btree");
  TEST_CHECK(getNumEntries(tree, &testint));
  ASSERT_EQUALS_INT(testint, numInserts, "number of entries in btree"); // Fixed Summer 2021 - Old: ASSERT_EQUALS_INT(numInserts,testint, "number of entries in btree");
...
testIndexScan (void)
{
      ...
      while((rc = nextEntry(sc, &rid)) == RC_OK)
  {
    ...
    ASSERT_EQUALS_RID(expRid, rid, "did we find the correct RID?");
      ...
      ASSERT_EQUALS_INT(numInserts, i + 1, "have seen all entries");
      closeTreeScan(sc);
